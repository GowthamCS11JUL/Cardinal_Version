  ...
  case UART_TARGET_RECEIVED: {
    ...
    if (param->command_active == 0) 
    {
     param->command_active = 1;
      param->cmd_start_time =Delay_Timer_Get(&delay);
    }
    ...
    /* ------------------------------------ */
      ...
      if (param->cmd_index < (MAX_CMD_LEN - 1)) {
        ...
        UART_Target_Transmit(param->uartTargetIndex, (uint8_t *)invalid_cmd,sizeof(invalid_cmd) - 1);
      }
    }
  ...
  for (uint8_t i = 0;i < MAX_NUM_CMDS; i++)
   {
    ...
    {
      if (memcmp(usb_data->cmd_buffer,usb_data->command_table[i].command,usb_data->cmd_len ) == 0) 
      {
...
{ 
  ...
  if (validate_usb_rx_data(usb_data) == ERROR_NONE) {
    ...
    if (executing_command->resp_len > 0) {
      UART_Target_Transmit( usb_data->uartTargetIndex,executing_command->response,executing_command->resp_len);
    }
  ...
  UART_Target_Transmit(  usb_data->uartTargetIndex, (uint8_t *)invalid_cmd, sizeof(invalid_cmd) - 1);
}
